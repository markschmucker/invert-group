...
class MyDiscourseClient(DiscourseClient):
    ...
    def user_summary(self, username):
        # includes number of likes and other data from public summary
        for attempt in range(10):
            try:
                return self._get(u"/u/{0}/summary.json".format(username))
            except DiscourseClientError, requests.ConnectionError:
                print 'user details failed; trying again'
                sleep(1)
    ...
    def get_username_by_email(self, email):
        data = self._get(u"/admin/users/list/all.json?email={0}".format(email))
        if data:
            return data[0]
